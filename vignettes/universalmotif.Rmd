...
seqLogo(motif["motif"], ic.scale = FALSE)
```
...
motif <- create_motif(c("AAGAAT", "ATCATA", "AAGTAA", "AACAAA", "ATTAAA",
                        "AAGAAT"), type = "PPM", pseudocount = 0)
seqLogo(motif["motif"], ic.scale = TRUE)
```
...
The total information content (IC) is based on alphabet length,
...
\begin{equation}
  Uncertainty = - \sum_{N=A}^{T} Prob_N \times{} \log{2}(Prob_N)
  (\#eq:posuncer)
\end{equation}
...
Finally, the get the height of each letter at each position, the final IC is
...
\begin{equation}
  IC_N = Prob_N \times{} \log{2}\left(\frac{Prob_N}{ProbBkg_N}\right)
  (\#eq:kldiv)
\end{equation}
...
## Converting from another package

## Converting from a universalmotif motif
