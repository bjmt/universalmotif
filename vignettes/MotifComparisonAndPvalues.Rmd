...
There are a couple of functions available in other Bioconductor packages which allow for motif comparison, such as `PWMSimilarity()` (`TFBSTools`) and `motifSimilarity()` (`PWMEnrich`). Unfortunately these functions are not designed for comparing large numbers of motifs. Furthermore they are restrictive in their option range. The `universalmotif` package aims to fix this by providing the `compare_motifs()` / `compare_motifs_lite()` function. Several other functions also make use of the core `compare_motifs()` / `compare_motifs_lite()` functionality, including `merge_motifs()` / `merge_motifs_lite()` and `view_motifs()` / `view_motifs_lite()`. (The `2` versions of these functions are newer, faster, simplified versions.) The following introduction of motif comparison is specifically concerning the original fully-featured `compare_motifs()`
...
l_2_df <- function(x) {
  ...
  d <- data.frame(key = character(), scores = numeric())
...
dres <- res6 %>%
          group_by(key, type) %>%
          summarise(mean = mean(scores, na.rm = TRUE), .groups = "drop")
...
## A faster alternative: `compare_motifs_lite()`

For DNA or RNA motif comparison that does not need the alternative
similarity metrics (EUCL, KL, ALLR, and so on), the score-aggregation
strategies, the IC-based filters, multifreq scoring, or the
JASPAR-derived `db.scores` lookup-table P-value path, the lighter
`compare_motifs_lite()` is generally much faster than `compare_motifs()`,
and it parallelises rather better across queries. It uses the
per-column Pearson correlation as its only similarity metric, computes
p-values from a TOMTOM-style empirical or parametric null PMF (with
both Bonferroni and Benjamini-Hochberg adjustment), and returns either
a square matrix or a long-format `data.frame`.

The function has eight formals (`motifs`, `compare.to`, `qvalue`,
`min.overlap`, `RC`, `null`, `bkg`, `matrix.out`, `nthreads`); see
`?compare_motifs_lite` for the details. There are two null-PMF modes:

- **`null = "empirical"`** (default): per-query column, score against
  every target-database column, histogram into 51 PCC bins, convolve
  to overlap length $L$. Cost per query scales with database size;
  best for databases with more than a few hundred total columns.
- **`null = "parametric"`**: enumerate the 56 compositions of A/C/G/T
  on a $K=5$ simplex grid, weight each by a Dirichlet-Multinomial PMF
  under $\alpha = 4 \times \mathtt{bkg}$, score against the query
  column. Per-query cost is constant in database size; best for small
...
hits <- compare_motifs_lite(motifs, compare.to = 1, qvalue = 0.05)
```

When `compare_motifs()` is called with arguments that map cleanly onto
`compare_motifs_lite()`'s feature set (PCC + sum strategy, the default IC
filters, no multifreq, no HTML report, no `db.scores`, and a DNA/RNA
alphabet), it emits a one-line hint pointing you at the faster
function. (Silence it with
`options(universalmotif.suggest.compare_motifs_lite = FALSE)`.)

### Speed comparison vs `compare_motifs()` and `yamtk cmp` {.unnumbered}

The table below records the wall-clock time for the three functions
on a single shared fixture: the first 50 (or 200) motifs of
HOCOMOCOv11, compared against themselves, with RC = TRUE,
min.overlap = 5, a uniform background, and the empirical null PMF. The
`compare_motifs_lite()` numbers are shown for both `matrix.out = "score"`
(which skips the p-value path entirely, since the best alignment is
chosen by score rather than by p-value) and `matrix.out = "pvalue"`
(the full significance computation, which is equivalent work to
`yamtk cmp`). Each cell shows `nthreads = 1 / nthreads = 4`. The
values are baked in here rather than recomputed at vignette build
time. These benchmarks were run on an MBP M5 under R 4.4.1.

| Workload | yamtk¹ | `compare_motifs()` | `compare_motifs_lite()` score² | `compare_motifs_lite()` pvalue |
|---|---:|---:|---:|---:|
...
The `score` / `pvalue` labels indicate the value of the `matrix.out`
argument.

¹ `yamtk cmp` is a dedicated C CLI tool; threading is by query motif.
The `compare_motifs_lite()` algorithm is a port of yamtk's.

² Score-only matrix mode skips the per-pair p-value lookup, which
otherwise dominates runtime on dense fixtures. This is safe because
the best alignment is selected by score, not p-value; p-values are
computed only as a post-hoc significance check on the already-chosen
alignment.

To summarise:

- `yamtk cmp` is the fastest of the three at every scale; the
  `compare_motifs_lite()` port stays within ~2× at 4 threads for score-only
  output and within ~12× for full p-value output.
- `compare_motifs_lite()` is 5-40× faster than `compare_motifs()` at
  4 threads on the same set, with the larger speedups in score-only
  mode. `compare_motifs()`'s p-value path uses pre-computed JASPAR
  score-distribution tables (`JASPAR2018_CORE_DBSCORES`) and a different
  null model, so the numbers are not directly comparable as
  "significance" but are comparable as "wall-clock to a similarity
  matrix".
- Threading scales 3-3.5× at 4 threads in all three universalmotif
  modes.
- For new code that does not need any of `compare_motifs()`'s
  advanced features, try `compare_motifs_lite()`.

# Merging motifs

The `universalmotif` package offers two ways to collapse similar
motifs into consensus motifs: `merge_motifs()` / `merge_similar()`,
and `merge_motifs_lite()` / `merge_similar_lite()` (the latter pair are
newer, built on `compare_motifs_lite()`, and use significance-based
clustering).

## A faster alternative: `merge_motifs_lite()` / `merge_similar_lite()`

For simple DNA/RNA motif merging tasks:

- `merge_motifs_lite(motifs)` aligns every motif against the highest-IC
  anchor via `compare_motifs_lite()` and column-averages them in a single
  shared frame.

- `merge_similar_lite(motifs, qvalue)` clusters motifs by statistical
  significance: two motifs are linked if their pairwise q-value
  meets the chosen `qvalue` cutoff. Clustering is by connected
  components on the resulting graph (deterministic, no linkage-method
  to choose). Each component is collapsed via `merge_motifs_lite()`.
...
`motif_tree_lite()` draws a sequence logo at every tip by default (`tiplogo = TRUE`), so the motifs themselves serve as the tip labels. The logos are aligned to a shared column frame (via `view_motifs_lite()`) so that homologous positions line up across tips.
...
## Tip logos are on by default; the default layout is now "rectangular".
motif_tree_lite(motifs, linecol = "none")
```

Use `tiplogo = FALSE` to turn the logos off, or `tiplogo.align = FALSE` to draw each tip logo independently instead of aligned to a common frame. Tip logos are only drawn on the `"rectangular"` and `"slanted"` layouts. The machinery is also exposed directly as the `geom_motif()` and `geom_logo()` `ggplot2` layers (and the `annotate_motif()` / `annotate_logo()` annotation helpers), so logos can be embedded in any `ggplot` construction.
...
Dynamic programming calculates the probability of each score without listing
every possible sequence. FIMO uses this approach [@grant11], and @gupta07
describes it in the context of motif comparison.

The algorithm works through the motif one position at a time. At each position,
it updates the possible total scores and their probabilities, using the
background frequencies of the letters. Once this score distribution is built,
it can answer many score or P-value queries without repeating the calculation.

Internally, each PWM entry is multiplied by 1000 and its decimal part is
dropped (truncation towards zero). The resulting integers are then added.
This puts scores on a grid with steps of 0.001. When you supply a score, it is
rounded up to the next grid point, or left unchanged if already on the grid.
The returned P-value is the probability of getting that score or a higher one:
`Pr(score >= query)`.

When you supply a P-value cutoff instead, the function returns the lowest
threshold on this grid for which the probability of a match at or above the
threshold is no greater than the cutoff. The threshold need not be a score
that a sequence can actually achieve. If even the highest-scoring match fails
the cutoff, the function returns `Inf` and scanning reports no hits.

These calculations use the truncated PWM scores, not the original scores at
full precision. Truncation can give different sequences the same score, or
change the order of very similar scores. This means that small score changes
can sometimes cause much larger P-value changes. This effect comes from the
score approximation, rather than small numerical errors when adding
probabilities.

Dynamic programming also needs finite PWM entries. If zero probabilities
produce `-Inf` scores, use pseudocounts to make the scores finite, or consider
the exhaustive method with `allow.nonfinite`.
...
Converting a score to a P-value and back may give a different score, because
several thresholds can select the same matches. What matters is that the
probability of a match at or above the returned threshold does not exceed the
requested cutoff.

### Which background does a P-value use?

Background frequencies affect both the PWM scores and the chance of seeing
each sequence. With the dynamic method and ordinary mononucleotide motifs
(`use.freq = 1`), supplying `bkg.probs` changes both: the function recalculates the PWM
using the new background, then uses that background to calculate probabilities.
It does not leave the PWM scores unchanged. The same sequence can therefore
have a different score under a different background.

Both scanners use the background stored in the motif; they do not automatically
estimate it from the sequences being scanned. To use a different background,
first convert the motif to a PPM using its original background. Then set the
new background and convert to a PWM:

```{r background-rebasing}
probabilities <- matrix(rep(c(.5, .25, .125, .125), 4), nrow = 4,
                        dimnames = list(c("A", "C", "G", "T"), NULL))
original <- create_motif(probabilities, type = "PPM", pseudocount = 0)
rebased <- convert_type(original, "PPM")
rebased["bkg"] <- c(A = .1, C = .4, G = .4, T = .1)
rebased <- convert_type(rebased, "PWM")
motif_pvalue(rebased, pvalue = .01)
```

Changing the `bkg` slot of a PWM alone does not update its scores: the
PPM-to-PWM conversion is needed too. The example above uses `use.freq = 1`.
For higher-order motifs, `use.freq` selects both the higher-order matrix and
its corresponding background probabilities.

The background sequences used in an enrichment test serve a different purpose:
they are the control group. Matching their composition to the target sequences
does not change the motif's PWM. Choose both the motif background and the
control sequences to suit the question, and check how the test behaves on
data with no enrichment. Composition matching alone does not guarantee
accurate P-values or false discovery rates.
...
knitr::kable(res, format = "markdown", digits = 22)
```
Both algorithms use truncated PWM scores rather than the original scores at
full precision. The exhaustive method also uses an approximation when it
splits a long motif into shorter pieces. Agreement between the methods on
short motifs does not mean that either always gives exact probabilities for
the original, full-precision PWM.
