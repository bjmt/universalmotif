...
for (m in metrics) {
  ...
  res2[[m]] <- as.numeric(as.dist(compare_motifs(y1, method=m, min.mean.ic=0,
                                                  min.overlap=99, score.strat="sum")))
  res3[[m]] <- as.numeric(as.dist(compare_motifs(y1, method=m, min.mean.ic=0,
                                                  min.overlap=99, score.strat="a.mean")))
  if (!m %in% c("PCC", "ALLR", "ALLR_LL"))
    res4[[m]] <- as.numeric(as.dist(compare_motifs(y1, method=m, min.mean.ic=0,
                                                    min.overlap=99, score.strat="g.mean")))
  res5[[m]] <- as.numeric(as.dist(compare_motifs(y1, method=m, min.mean.ic=0,
                                                  min.overlap=99, score.strat="median")))
}
...
res3 <- gather(as.data.frame(res3))
colnames(res3) <- c("key", "score")
res4 <- gather(as.data.frame(res4))
colnames(res4) <- c("key", "score")
res5 <- gather(as.data.frame(res5))
colnames(res5) <- c("key", "score")
...
res5$type <- "Median"
# res4 <- rbind(res, res2)
res6 <- rbind(res, res3, res4, res5)

dres <- res6 %>%
          group_by(key, type) %>%
          summarise(mean = mean(score))

ggplot(res6, aes(x = score, fill = type)) +
...
try_all <- function(motifs, ...) {
  ...
  methods <- c("PCC", "EUCL", "SW", "KL", "ALLR",
               "BHAT", "HELL", "IS", "SEUCL", "MAN", "ALLR_LL")
  ...
  res <- data.frame(type = c("similarity", "distance", 
