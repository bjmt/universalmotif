...
into consensus motifs: `merge_motifs()` / `merge_similar()` (older score-
threshold clustering via hierarchical clustering on a distance matrix)
and `merge_motifs2()` / `merge_similar2()` (new, built on
...
available to `merge_motifs2()` / `merge_similar2()`,
a one-line hint is emitted pointing at the
