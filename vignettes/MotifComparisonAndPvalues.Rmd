...
There are a couple of functions available in other Bioconductor packages which allow for motif comparison, such as `PWMSimilarity()` (`TFBSTools`) and `motifSimilarity()` (`PWMEnrich`). Unfortunately these functions are not designed for comparing large numbers of motifs. Furthermore they are restrictive in their option range. The `universalmotif` package aims to fix this by providing the `compare_motifs()` function. Several other functions also make use of the core `compare_motifs()` functionality, including `merge_motifs()` and `view_motifs()`.
...
For DNA/RNA motif comparison that does not need alternative similarity
metrics (EUCL, KL, ALLR, …), score-aggregation strategies, IC-based
filters, multifreq scoring, or the JASPAR-derived `db.scores`
lookup-table P-value path, the lighter `compare_motifs2()` function is
generally much faster than `compare_motifs()` and parallelises better
across queries. It uses Pearson correlation per column as the only
similarity metric, computes p-values from a TOMTOM-style empirical or
parametric null PMF (with Bonferroni + Benjamini-Hochberg adjustment),
and returns either a square matrix or a long-format `data.frame`. The
default surface is aligned with the command-line tool `yamtk cmp`
...
`?compare_motifs2` for details. Two null-PMF modes are supported:
...
`compare_motifs2()`'s feature set (PCC + sum strategy, default IC
filters, no multifreq, no HTML report, no `db.scores`, DNA/RNA
alphabet), it emits a one-line hint pointing at the faster function.
Silence with
`options(universalmotif.suggest.compare_motifs2 = FALSE)`.

### Speed comparison vs `compare_motifs()` and `yamtk cmp` {.unnumbered}

The table below records wall-clock time for the three functions on the
same fixture: the first 50 (or 200) motifs of HOCOMOCOv11 compared
against themselves, RC = TRUE, min.overlap = 5, uniform background,
empirical null PMF. `compare_motifs2()` numbers are shown for both
`matrix.out = "score"` (which skips the p-value path entirely, since
the best alignment is chosen by score not by p-value) and
`matrix.out = "pvalue"` (full significance computation, equivalent
work to `yamtk cmp`). Each cell shows `nthreads = 1 / nthreads = 4`.
...
time. These benchmarks were run using a MBP M5 running R 4.4.1.
...
- `compare_motifs2()` is **5-40× faster than `compare_motifs()`** at
  4 threads on the same fixture, with the larger speedups in score-only
...
- For new code that does not need any of `compare_motifs()`'s
  advanced features, prefer `compare_motifs2()`.

# Merging motifs

The `universalmotif` package offers two ways to collapse similar motifs
into consensus motifs: `merge_motifs()` / `merge_similar()` (v1, score-
threshold clustering via hierarchical clustering on a distance matrix)
and `merge_motifs2()` / `merge_similar2()` (v2, built on
`compare_motifs2()`, with significance-based clustering).

## A faster alternative: `merge_motifs2()` / `merge_similar2()`

For users on DNA/RNA motifs who want the v2-stack defaults:
...
- `merge_motifs2(motifs)` aligns every motif against the highest-IC
  ...
  shared frame. This is anchor-based rather than v1's iterative
  pairwise scheme, so the result is independent of input order.
...
When a `merge_motifs()` / `merge_similar()` call uses only features
that map cleanly onto v2, a one-line hint is emitted pointing at the
leaner function. Silence with
`options(universalmotif.suggest.merge_motifs2 = FALSE)` and
