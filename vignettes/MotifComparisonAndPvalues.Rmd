...
a square matrix or a long-format `data.frame`. Its default surface is
aligned with the command-line tool
...
benchmark script lives at `benchmarks/compare_motifs2_vignette.log` in
the package repository, and the values are baked in here rather than
recomputed at vignette build time. These benchmarks were run on an
MBP M5 under R 4.4.1.
...
`cm2()` is shorthand for `compare_motifs_lite()`; the `score` / `pvalue`
labels indicate the value of the `matrix.out` argument.
...
- `merge_similar_lite(motifs, qvalue)` clusters motifs by statistical
  ...
  meets the user's `qvalue` cutoff. Clustering is by connected
