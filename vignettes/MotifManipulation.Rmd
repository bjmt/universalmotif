...
The `universalmotif` package includes the `merge_motifs()` / `merge_motifs2()` functions to combine motifs (the latter is a newer, leaner version). Motifs are first aligned, and the best match found before the motif matrices are averaged. The implementation for this is identical to that used by `compare_motifs()` / `compare_motifs2()` (see the [motif comparisons vignette](MotifComparisonAndPvalues.pdf) for more information).
...
This functionality can also be automated to reduce the number of overly similar motifs in larger datasets via the `merge_similar()` / `merge_similar2()` functions.
...
Comparison and merging parameters can be fine-tuned as users wish. See the `compare_motifs()` / `compare_motifs2()` and `merge_motifs()` / `merge_motifs2()` documentation for more details, as well as the "Motif comparisons and P-values" vignette.
...
There are several packages which offer motif visualisation capabilities, such as `seqLogo`, `motifStack`, and `ggseqlogo`. The `universalmotif` package has its own implementation via the function `view_motifs()` / `view_motifs2()`, which renders motifs using the `ggplot2` package (similar to `ggseqlogo`). Here I will briefly show how to use these to visualise `universalmotif` class motifs.
...
The `view_motifs()` / `view_motifs2()` functions generates `ggplot` objects; feel free to manipulate them as such. For example, flipping the position numbers for larger motifs (where the text spacing can become tight):
...
A large number of options are available for tuning the way motifs are plotted in `view_motifs()` / `view_motifs2()`. Visit the documentation for more information.
...
The `motifStack` package allows for a number of different motif stacking visualisations. The `universalmotif` package, while not capable of emulating most of these, still offers basic stacking via `view_motifs()` / `view_motifs2()`. The motifs are aligned using `compare_motifs()` / `compare_motifs2()`.
...
The logo plotting capabilities of `view_motifs()` / `view_motifs2()` can be used for any kind of arbitrary text logo. All you need is a numeric matrix (the heights of the characters), with the desired characters as row names. The following example is taken from the `view_logo()` documentation.
