...
abstract: >
  The universalmotif package offers a number of functions to manipulate motifs. These are introduced and explored here, including those relating to: import, export, motif modification, creation, visualisation, and other miscellaneous utilities.
vignette: >
...
This vignette will introduce the `universalmotif` class and its structure, the import and export of motifs in R, basic motif manipulation, creation, and visualisation. For an introduction to sequence motifs, see the [introductory](IntroductionToSequenceMotifs.pdf) vignette. For sequence-related utilities, see the [sequences](SequenceSearches.pdf) vignette. For motif comparisons and P-values, see the [motif comparisons and P-values](MotifComparisonAndPvalues.pdf) vignette. For the multi-source curation step that picks up where these import functions leave off (building a single, deduplicated, and annotated motif library out of several public databases), see the [curated motif database](MotifDatabaseCuration.pdf) vignette.

# The universalmotif class and conversion utilities

## The universalmotif class

The `universalmotif` package stores motifs using the `universalmotif` class. The most basic `universalmotif` object exposes the `name`, `alphabet`, `type`, `strand`, `icscore`, `consensus`, and `motif` slots; furthermore, the `pseudocount` and `bkg` slots are also stored but not shown. Motifs can be PCM, PPM, PWM, ICM, or CWM type; CWMs store contributions rather than a probability-based motif model.
...
* `type`: motif 'type', one of PCM, PPM, PWM, ICM, CWM; see the [introductory](IntroductionToSequenceMotifs.pdf) vignette
...
* `pseudocount`: this value is added to the motif matrix during certain type conversions; this is necessary to avoid `-Inf` values from appearing in PWM type motifs
...
Below the exposed metadata slots, the actual 'motif' matrix is shown. Each position is its own column, with row names showing the alphabet letters and column names showing the consensus letter at each position.
...
The `convert_motifs` function is embedded in most of the `universalmotif` functions, meaning that compatible motif classes from other packages can be used without needing to manually convert them first. However, keep in mind some conversions are final. Furthermore, internally, all motifs regardless of class are handled as `universalmotif` objects, even if the returning class is not. This will result in at times slightly different objects (though usually no information should be lost).
...
Use `convert_type()` to transition between PCM, PPM, PWM, and ICM types.
Back conversion from an ICM calculated with sample correction or relative
entropy can be inaccurate; the object does not record enough information for
`convert_type()` to detect that history. CWM conversion is separately lossy:
conversion to PPM normalises absolute contributions, discarding their signs
and scale. Other conversions from CWM go through this PPM, and conversion
from a probabilistic type back to CWM is not supported.
...
Changing the `pseudocount` slot is not itself a request to resmooth an existing
matrix. Use `normalize()` when you intend to apply smoothing to the motif, or
set the argument on the conversion that needs it. Converting to a type the
motif already has can be a no-op. For count-to-probability conversion the site
count controls the relative influence of the pseudocount; an unknown count is
not the same as zero observations. Likewise, converting an unknown-count PPM
to a PCM uses an assumed count (100 by default), not a measured sample size.
Retain that distinction when interpreting exported counts.

For zero-probability entries, an unadjusted PWM can contain `-Inf`. Dynamic
P-value calculations require finite scores and may apply a pseudocount
correction with a message. Check these settings before comparing results from
different conversion or scanning paths. CWMs instead store signed contributions;
their column sums must not be interpreted as site counts or probabilities.
...
The `universalmotif` package includes the `merge_motifs()` / `merge_motifs_lite()` functions to combine motifs (the latter is a newer, leaner version). Motifs are first aligned, and the best match found before the motif matrices are averaged. The implementation for this is identical to that used by `compare_motifs()` / `compare_motifs_lite()` (see the [motif comparisons vignette](MotifComparisonAndPvalues.pdf) for more information).
...
This functionality can also be automated to reduce the number of overly similar motifs in larger datasets via the `merge_similar()` / `merge_similar_lite()` functions.
...
Comparison and merging parameters can be fine-tuned as desired. See the `compare_motifs()` / `compare_motifs_lite()` and `merge_motifs()` / `merge_motifs_lite()` documentation for more details, as well as the "Motif comparisons and P-values" vignette.
...
If you wish, it is easy to create random motifs. The values within the motif are generated using `rgamma()` to avoid creating low information content motifs. If background probabilities are not provided, then they are generated with `rpois()`.
...
There are several packages which offer motif visualisation capabilities, such as `seqLogo`, `motifStack`, and `ggseqlogo`. The `universalmotif` package has its own implementation via the function `view_motifs()` / `view_motifs_lite()`, which renders motifs using the `ggplot2` package (similar to `ggseqlogo`). Here I will briefly show how to use these to visualise `universalmotif` class motifs.
...
The `view_motifs()` / `view_motifs_lite()` functions generates `ggplot` objects; feel free to manipulate them as such. For example, flipping the position numbers for larger motifs (where the text spacing can become tight):
...
A large number of options are available for tuning the way motifs are plotted in `view_motifs()` / `view_motifs_lite()`. Visit the documentation for more information.
...
The `motifStack` package allows for a number of different motif stacking visualisations. The `universalmotif` package, while not capable of emulating most of these, still offers basic stacking via `view_motifs()` / `view_motifs_lite()`. The motifs are aligned using `compare_motifs()` / `compare_motifs_lite()`.
...
The logo plotting capabilities of `view_motifs()` / `view_motifs_lite()` can be used for any kind of arbitrary text logo. All you need is a numeric matrix (the heights of the characters), with the desired characters as row names. The following example is taken from the `view_logo()` documentation.
...
Based on the PPM representation, all three of CAAAACC, CTTTTCC, and CTATACC are equally likely. Though looking at the starting sequences, should CTATACC really be considered equally likely? For transcription factor binding sites, this sometimes is not the case. By incorporating this type of information into the motif, it can allow for increased accuracy in motif searching. A few example implementations of this include: TFFM by @tffm, BaMM by @bamm, and KSM by @ksm.
...
Feel free to add columns as well. You can add one-dimensional vectors which will be added to the `extrainfo` slots of motifs. (Note that they will be coerced to character vectors!)
...
For DNA, RNA and AA motifs, the `universalmotif` package will automatically generate a `consensus` string slot. Furthermore, `create_motif()` can generate motifs from consensus strings. The internal functions for these have been made available:
...
## Show matches scoring at least 80% of this motif's maximum:
get_matches(examplemotif, .8 * motif_score(examplemotif, 1))
...
Additionally, `position_icscore()` can be used to get the total information content per position:
