...
There are several packages which offer motif visualization capabilities, such as `seqLogo`, `Logolas`, `motifStack`, and `ggseqlogo`. The `universalmotif` package has its own implementation which is similar to the one offered by `ggseqlogo` via the function `view_motifs()`. Here I will briefly show how to use these to visualize `universalmotif` class motifs.
...
## Logolas:
## NOTE: the Logolas package is no longer available in Bioconductor 3.13 and later
