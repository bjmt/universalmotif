...
Since biological sequences are usually contained in \Rclass{XStringSet} class objects, `get_bkg()` and `shuffle_sequences()` are designed to work with such objects. For cases when strings are not \Rclass{XStringSet} objects, the following functions are available:
...
enrich_motifs(ArabidopsisMotif, ArabidopsisPromoters, shuffle.k = 3,
              threshold = 0.001, RC = TRUE)[[1]]
```

Here we can see that the motif is significantly enriched in the target sequences. The `Pval.hits` was calculated by calling `fisher.test` from the `r Rpackage("stats")` package. Another type of test is available which looks for positional enrichment, i.e. whether one area in the target sequences is favoured.
...
enrich_motifs(ArabidopsisMotif, ArabidopsisPromoters, shuffle.k = 3,
              search.mode = "positional", RC = TRUE)[[1]]
```

In this case however no such bias exists in the target sequences. There are a number of tests available for positional enrichment; see `?enrich_motifs`. Furthermore, the `motif_peaks()` function can be used to identify non-random motif clusters; see the next section for details.
...
The `r Biocpkg("universalmotif")` package provides a simple wrapper to the powerful motif discovery tool \software{MEME} [@meme3]. To run an analysis with \software{MEME}, all that is required is a set of \Rclass{XStringSet} class sequences (defined in the `r Biocpkg("Biostrings")` package), and `run_meme()` will take care of running the program and reading the output for use within \R{}.

The first step is to check that R can find the \software{MEME} binary in your `$PATH` by running `run_meme()` without any parameters. If successful, you should see the default \software{MEME} help message in your console. If not, then you'll need to provide the complete path to the \software{MEME} binary. There are two options:
...
Once the sequences are ready, there are few important options to keep in mind. One is whether to conserve the output from \software{MEME}. The default is not to, but this can be changed by setting the relevant option:
...
Finally, choose how you'd like the data imported into \R{}. Once the \software{MEME} program exits, `run_meme()` will import the results into \R{} with `read_meme()`; at this point you can decide if you want just the motifs themselves (`readsites = FALSE`) or if you'd like the original sequence sites as well (`readsites = TRUE`, the default).

There are a wealth of other \software{MEME} options available, such as the number of desired motifs (`nmotifs`), the width of desired motifs (`minw`, `maxw`), the search mode (`mod`), assigning sequence weights (`weights`), using a custom alphabet (`alph`), and many others. See the output from `run_meme()` for a brief description of the options, or visit the [online manual](http://meme-suite.org/doc/meme.html) for more details.
