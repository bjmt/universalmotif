...
# Plotting aligned sequences as motifs

By making use of `create_motif()` and `view_motifs()`, one can easily plot the distribution of letter composition of aligned sequences. Using the higher order `multifreq` motif representation, this can even be done, for example, for dinucleotide or trinucleotide distributions across sequences. In the following example the last 20 bases (a small number is used for the sake of reducing the size of the vignette; try running this example using larger values!) before the TSS of the included `ArabidopsisPromoters` data object is plotted as mono- and di-letter PPMs and ICMs. Since the higher order representations will look quite compact, the space between k-lets is increased by setting `x.spacer` to make clearer the distinction between k-lets.
...
## First, isolate the last 20bp:
seqs <- narrow(ArabidopsisPromoters, 980, 1000)

## Then represent the sequences as a motif with a filled multifreq slot:
seqs.mot <- create_motif(seqs, add.multifreq = 2:3)

mytheme <- function() theme_void() + theme(legend.position = "none")

## Plot all combinations of PPM and ICM for mono- and di-lets
cowplot::plot_grid(ncol = 1, rel_heights = c(rep(c(1, 0.1), 3), 1),
  view_motifs(seqs.mot, use.type = "PPM", sort.positions = TRUE) + mytheme(),
  NULL,
  view_motifs(seqs.mot, use.type = "PPM", use.freq = 2,
    sort.positions = TRUE, x.spacer = 0.15) + mytheme(),
  NULL,
  view_motifs(seqs.mot, use.type = "ICM") + mytheme() + ylim(c(0, 0.5)),
  NULL,
  view_motifs(seqs.mot, use.type = "ICM", use.freq = 2,
    x.spacer = 0.15) + mytheme() + ylim(c(0, 1))
)
```

You could also use this trick to plot the sequence background surrounding motif hits. In the following example, a new motif object is created which includes the surrouding sequence of motif hits and plotted.
...
## Collect our motif hit results as a GRanges object
hits <- scan_sequences(ArabidopsisMotif, ArabidopsisPromoters, RC = TRUE,
  return.granges = TRUE, no.overlaps = TRUE)

## Resize to include surrounding sequence
hits <- suppressWarnings(trim(resize(hits, 35, fix = "center")))

## Remove hits which are too close to the edges as these cannot be used
hits <- hits[width(hits) == 35, ]

## Retrieve the sequences
seqs <- ArabidopsisPromoters[hits]

## Create the motif
new.mot <- create_motif(seqs, add.multifreq = 2)

## Plot it, highlighting the actual motif
cowplot::plot_grid(ncol = 1,
  view_motifs(new.mot, show.positions = FALSE, use.type = "ICM") +
    annotate("rect", ymin = 0, ymax = 2, xmin = 9.5, xmax = 25.5,
      alpha = 0.1, colour = "blue", fill = "pink") +
    ggtitle("Motif: 1-letter"),
  view_motifs(new.mot, show.positions = FALSE, use.type = "ICM", use.freq = 2) +
    annotate("rect", ymin = 0, ymax = 4, xmin = 9.5, xmax = 24.5,
      alpha = 0.1, colour = "blue", fill = "pink") +
    ggtitle("Motif: 2-letter")
)
```
