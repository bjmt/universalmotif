...
`enrich_motifs()` and `enrich_motifs2()` test whether each motif is
...
internally via `scan_sequences2()`, and so is DNA/RNA only.
...
than a P-value, the lighter `scan_sequences2()` is generally faster,
...
The default surface of `scan_sequences2()` is deliberately minimal
...
cleanly onto `scan_sequences2()`'s feature set, it emits a one-line
...
`scan_sequences2()`, and `motifmatchr` in `positions` mode all agree
...
- For new code that does not need any of `scan_sequences()`'s
  advanced features, I would generally reach for `scan_sequences2()`.
...
`enrich_motifs()`, with defaults that mirror those of the
command-line tool [`yamtk`](https://github.com/bjmt/yamtk). It is
built on top of `scan_sequences2()`, and so inherits its speed
advantages.
...
(a per-site comparison), while `enrich_motifs2()` defaults to
`test = "seqs"` (a per-sequence comparison, matching the `yamtk enr`
default). Setting `test = "sites"` makes `enrich_motifs2()` mirror the
...
The output is a plain `data.frame` whose 13 columns mirror the TSV
from `yamtk enr` one-for-one: `motif`, `motif.i`, `consensus`,
...
onto `enrich_motifs2()`, a one-line hint is again emitted pointing
you at the leaner function. (Silence it with
`options(universalmotif.suggest.enrich_motifs2 = FALSE)`.)

## _De novo_ motif discovery

`motif_finder()` is a _de novo_ motif discovery function whose
defaults, again, mirror those of the command-line tool
[`yamtk`](https://github.com/bjmt/yamtk). For each motif width in a
user-controlled range, it enumerates the over-represented k-mer seeds
...
`alphabet`, and so on) joined to the yamtk-specific stats (`rank`,
...
`view_motifs2()`, `compare_motifs2()`, `write_meme()`, and the rest of
the package.
...
`scan_sequences2()`. When the input is a `GRanges` that has its
`seqlengths()` set (which `scan_sequences2()` does for you
