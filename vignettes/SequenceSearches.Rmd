...
preserving its own k-let composition. That's appropriate when the
input set is roughly homogeneous, but it leaves a confounder: if the
input sequences have a systematic composition bias relative to the
genome (GC-rich ChIP-seq peaks, AT-rich promoters, ...), shuffling
alone won't control for it. `match_bkg()` instead samples real
sequences from a larger universe that match the input on GC
fraction and length, the HOMER-style binned approach.
...
This overlays GC-fraction and length density curves for target and
matched background in a two-panel ggplot.

`match_bkg()` complements `shuffle_sequences()`; neither replaces
the other. Use a shuffle when you want a per-sequence k-let-preserving
null; use a matched background when you want to control for
composition bias relative to a genomic universe. For reproducibility,
call `set.seed()` before `match_bkg()`.

## Generating ground-truth positive sequences with `implant_motifs()`

When benchmarking a motif discovery, scanning, or enrichment pipeline,
you need positive sequences with known motif instances at known
positions so you can score recall and precision against an answer
key. `implant_motifs()` produces these: it takes a set of motifs and
host sequences, samples one instance per implant column-by-column from
the motif's PPM, and overwrites the bases at the chosen position.
Three insertion modes are supported: a fixed `n.per.seq` count,
a Poisson `rate` per base pair, or an explicit list of `positions`.
Optional knobs (`centre.bias`, `min.spacing`, `strand`) let you mimic
ChIP-seq-style central enrichment or enforce non-overlapping placement.
...
## Visualise positional density
peaks <- motif_peaks(out, motif = ArabidopsisMotif)
plot_motif_peaks(peaks)
```

`return.indices = FALSE` (default) returns only the modified
`XStringSet`. `return.indices = TRUE` returns a list with two
elements: `sequences` (the modified `XStringSet`) and `indices` (a
`data.frame` with columns `sequence.i`, `motif.i`, `start`, `width`,
`strand`, `planted`).

## Motif pair co-occurrence with `motif_coocc()`

`enrich_motifs()` / `enrich_motifs2()` test whether each motif is
enriched on its own. But transcription factors often bind as
heterodimers or cooperate at composite cis-regulatory modules, so the
next-most-useful question is: do any motif pairs co-occur in the
same sequences more often than chance? `motif_coocc()` answers that.
For every pair of input motifs it builds the 2x2 contingency table of
per-sequence presence/absence and runs a one-sided Fisher's exact
test, then BH-corrects across all tested pairs.
...
Two input paths are supported. The default (above) scans internally
via `scan_sequences2()` and is DNA/RNA only. Alternatively you can
hand the function a precomputed hit table:

```{r, eval = FALSE}
hits <- scan_sequences2(motifs, seqs, pvalue = 1e-3)
co   <- motif_coocc(motifs, hits = hits, n.sequences = length(seqs))
```

The hit-table path accepts any alphabet (AA, custom); once a
hit table exists, co-occurrence is pure set arithmetic on
`(motif.i, sequence.i)` integers.

Set `max.distance = N` to additionally report two descriptive
columns: `both.clustered` (the subset of co-occurring sequences with
an `(A, B)` hit pair within `N` bp) and `median.distance` (the
median nearest-pair spacing in those sequences). The Fisher p-value
is unchanged, it always tests "do A and B occur in the same
sequences more often than chance?" on the unfiltered 2x2. The
spatial columns are interpretive aids for flagging heterodimer-like
arrangements rather than a separate statistical test.
...
Two scanning-related functions are provided: `scan_sequences()` / `scan_sequences2()` and `enrich_motifs()` / `enrich_motifs2()`. The latter simply runs `scan_sequences()` / `scan_sequences2()` twice on a set of target and background sequences. Given a motif of length `n`, `scan_sequences()` / `scan_sequences2()` considers every possible `n`-length subsequence in a sequence and scores it using the PWM format. If the match surpasses the minimum threshold, it is reported. This is the case regardless of whether one is scanning with a regular motif, or using the higher-order (`multifreq`) motif format (the `multifreq` matrix is converted to a PWM) with the older `scan_sequences()`.
...
## A faster alternative: `scan_sequences2()`

For DNA/RNA scans that don't need multifreq scoring, gapped motifs,
q-values, exhaustive P-values, or any
threshold type other than P-value, the lighter `scan_sequences2()`
function is generally faster and parallelises better across motifs.
It uses the same C++ scanner internally, computes per-hit P-values
via the same FIMO-style dynamic programming algorithm, and returns
the same hit set, but skips extra work
that `scan_sequences()` does to support its broader feature set.

The default surface of `scan_sequences2()` is intentionally minimal
(six formals: `motifs`, `sequences`, `pvalue = 1e-4`, `RC = TRUE`,
`nthreads = 1`, `return.granges = NULL`). When `GenomicRanges` is
installed, the default output is a `GRanges`; otherwise it falls back
to a `data.frame`. Coordinates always satisfy `start <= end`
regardless of strand (matching BED / GFF / `GRanges` convention), and
the `match` column on `-`-strand hits is the reverse complement of
the sequence substring, i.e. the matched orientation. Hit-level
deduplication using a greedy algorithm is
available via `no.overlaps = TRUE`, and is also exposed as the
standalone `dedup_hits()` function for post-processing.

```{r, eval = FALSE}
## scan_sequences2() with all defaults
hits <- scan_sequences2(ArabidopsisMotif, ArabidopsisPromoters)
...
When `scan_sequences()` is called with arguments that map cleanly onto
`scan_sequences2()`'s feature set, it emits a one-line hint pointing at
the faster function. This can be silenced with
`options(universalmotif.suggest.scan_sequences2 = FALSE)`.

### Speed comparison vs `motifmatchr` and `yamtk` {.unnumbered}

To put the trade-offs in perspective, the table below records
end-to-end wall-clock time for the four scanners on the same fixture:
the first 50 motifs of HOCOMOCOv11 against random DNA at five sizes,
P-value cutoff $5 \times 10^{-5}$, both strands, uniform background.
`motifmatchr::matchMotifs()` is included in two output modes -- its
default `out = "matches"` (per `(motif, sequence)`, strictly less
work than the other three tools) and `out = "positions"` (per
position, directly comparable to the others). Hit sets across
the cli tool [`yamtk`](https://github.com/bjmt/yamtk),
`scan_sequences()`, `scan_sequences2()`, and `motifmatchr` in
`positions` mode agree to within ~5% (due to a drift between
`yamtk`'s continuous-PPM PWM construction and `universalmotif`'s
PCM-rounding). Reported times are in seconds; each cell
shows `nthreads = 1 / nthreads = 4` where threading applies.
These benchmarks were run using a MBP M5 running R 4.4.1.

| Workload | yamtk | mm matches¹ | mm positions² | scan_sequences2 | scan_sequences³ |
|---|---:|---:|---:|---:|---:|
...
`scan_sequences2()`. `motifmatchr` is single-threaded from the R API
in both modes.

³ `scan_sequences(..., calc.pvals = TRUE)`, which performs the same
work per hit as `scan_sequences2()`. The older `mapply`-based dispatch
in `scan_sequences()` parallelises poorly for short-sequence
workloads; this is the main cost that
`scan_sequences2()` eliminates.
...
- `scan_sequences2()` reaches within ~3× of `yamtk scan` at 4 threads
  on long-sequence workloads, and is 2-3× faster than
  `scan_sequences(calc.pvals = TRUE)` at 4 threads across every
  configuration tested.
- For equivalent-work (per-position) scanning, `scan_sequences2()` is
...
- For new code that does not need any of `scan_sequences()`'s
  advanced features, prefer `scan_sequences2()`.
...
## A faster alternative: `enrich_motifs2()`

For users who only need the common case (DNA/RNA motifs, P-value
hit threshold, Fisher's exact test, Benjamini-Hochberg q-values),
`enrich_motifs2()` offers a stripped-down counterpart to
`enrich_motifs()` whose defaults mirror the command-line tool
[`yamtk`](https://github.com/bjmt/yamtk). It is built on
`scan_sequences2()` and inherits its speed advantages.
...
enrich_motifs2(ArabidopsisMotif, ArabidopsisPromoters,
               pvalue = 1e-3, qvalue = 0.1, rng.seed = 1)
```

The output is a plain `data.frame` whose 13 columns mirror `yamtk enr`'s
TSV one-for-one: `motif`, `motif.i`, `consensus`,
...
When a `enrich_motifs()` call uses only features that map
cleanly onto `enrich_motifs2()`, a one-line hint is emitted
pointing at the leaner function. Silence with
`options(universalmotif.suggest.enrich_motifs2 = FALSE)`.

## _De novo_ motif discovery

`motif_finder()` is a _de novo_ motif discovery function.
Its defaults mirror the command-line tool
[`yamtk`](https://github.com/bjmt/yamtk). For each motif width in a
user-controlled range it enumerates over-represented k-mer seeds
(Fisher's exact test on per-sequence presence vs. a background set),
aligns Hamming-distance-1 neighbours of the best seed to form a PPM,
refines the PPM via two re-scan passes, accepts the motif if its
Fisher's exact p-value passes `stop.pvalue`, masks the covered
positions and repeats up to `nmotifs` times per width. After all widths
complete, motifs are cross-width deduplicated, BH-adjusted, and
IC-trimmed at the flanks.
...
The return shape is a `universalmotif_df`: one row per discovered motif,
the motif S4 objects in the `motif` list-column, and the standard
`to_df()` slot columns (`name`, `consensus`, `alphabet`, etc.) joined
to the yamtk-specific stats (`rank`, `width`, `seqs_pos`, `seqs_neg`,
`sites_pos`, `sites_neg`, `n_pos`, `n_neg`, `pvalue`, `qvalue`). Use
`to_list()` to recover a plain list of `universalmotif` S4 objects,
which plug directly into `view_motifs2()`, `compare_motifs2()`,
`write_meme()`, and the rest of the package.

Discovery widths default to 6-15; the per-width motif cap defaults to
10; the result-level q-value filter defaults to 1e-3. Set
`qvalue = 1` to return all discovered motifs regardless of
significance. Background sequences are auto-generated by k=2 shuffling
of the input unless supplied via `bkg.sequences`.

## Positional enrichment with `motif_peaks()`

`motif_peaks()` tests whether motif hits cluster non-uniformly along the
input sequences. It implements an analytical
[CentriMo](https://doi.org/10.1093/nar/gks433)-style test: under the
null that hit positions are uniform over `[1, seq.length]`, the count
of hits in a candidate window follows a binomial distribution. The
function reports the most-enriched window per motif, Bonferroni-
corrected across the windows tested, then BH-adjusted across motifs.
...
The returned `data.frame` carries one row per significant motif plus a
list-column `centers` of best-hit centre positions per sequence;
`plot_motif_peaks()` consumes that column directly, so users don't
need to thread their original scan results through again.

Input is polymorphic: `motif_peaks()` accepts either a `data.frame`
or a `GRanges` from `scan_sequences()` or `scan_sequences2()`. When
the input is a `GRanges` with `seqlengths()` set (which
`scan_sequences2()` does automatically), `seq.length` is inferred;
...
_Note: In the time since the inception of the `run_meme()` function, Spencer Nystrom (a contributor to `universalmotif`) has created the `memes` package as an interface to much of the `MEME` suite. It is fully interoperable with the `universalmotif` package and provides a much more convenient way to run `MEME` programs from within `R`. Install it from Bioconductor with `BiocManager::install("memes")`._
