...
```
!!!Cite the nature paper!!!

This P-value can be further refined to obtain either a false discovery rate (FDR) or a Bonferroni-adjusted P-value. The former is calculated as the _ratio of null hits to observed hits in a set of sequences_, meaning that the minimum threshold for any FDR cannot be known _a priori_. Instead, the desired FDR value is first used as a P-value to calculate a threshold with `motif_pvalue()`, and then the final results are filtered based on their calculated FDR.

```{r}
data(ArabidopsisMotif)
data(ArabidopsisPromoters)

motif.len <- ncol(ArabidopsisMotif)
max.hits <- sum(width(ArabidopsisPromoters) - motif.len + 1)
max.hits

## Let us imagine calculating a FDR for a motif hit with a score of 11.
## We also need to know the number of hits with a score greater than our
## motif hit of interest. For this motif and set of sequences, this is 
## about 66.

observed.hits <- 67

pvalue <- motif_pvalue(ArabidopsisMotif, score = 11)
pvalue

null.hits <- pvalue * max.hits
null.hits

FDR <- null.hits / observed.hits
FDR
```

One additional correction is applied to the FDR when actually calculated after scanning: to account for the fact that the FDR is not guaranteed to always increase as it is calculated for lower and lower scoring hits, for any particular motif hit the maximum FDR of all FDRs of all motif hits with an equal or higher score is used.

Q-values on the other hand are calculated as the _P-value multiplied by the maximum number of possible motif hits in a set of sequences_. As one might imagine, this is an extremely strict P-value correction method which can only be used for smaller sets of sequences. Generally, the miminum possible P-value for any motif is much too large to survive being multiplied by the number of possible motif hits in, for example, an entire genome.

```{r}
## The corresponding Q-value for a score of 11:

qvalue <- pmin(pvalue * max.hits, 1)
qvalue

## So, what minimum score could give a significant Q-value?
q05 <- 0.05 / max.hits
q05
motif_pvalue(ArabidopsisMotif, pvalue = q05)

## Now imagine calculating a Q-value for the minimum possible P-value in
## an entire genome:

pvalue2 <- motif_pvalue(ArabidopsisMotif, motif_score(ArabidopsisMotif, 1) - 0.01)
pvalue2

Arabidopsis.max.hits <- 120000000

qvalue2 <- pmin(pvalue2 * Arabidopsis.max.hits, 1)
qvalue2
```
