...
Set `max.distance = N` to tighten "both present" to "both within
`N` bp in the same sequence", a SpaMo-style spatial constraint for
heterodimer / composite-site discovery. The pair table then carries
an extra `median.distance` column reporting the median nearest-pair
spacing across co-occurring sequences.

`plot_motif_coocc()` renders the result as a clustered tile heatmap
of `-log10(qvalue)` (or `odds_ratio`) for quick visual triage:
