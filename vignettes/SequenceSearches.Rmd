...
the motif's PPM, and overwrites the host bases at the chosen position.
...
## Plant one instance per sequence; recover ground-truth positions.
## Call set.seed() before each invocation for reproducible sampling.
set.seed(1)
out   <- implant_motifs(ArabidopsisMotif, hosts, n.per.seq = 1)
set.seed(1)
truth <- implant_motifs(ArabidopsisMotif, hosts, n.per.seq = 1,
                        return.indices = TRUE)
head(truth)
## Quantify recovery
...
By default the function returns the modified sequences. Re-call with
`return.indices = TRUE` after the same `set.seed()` to recover the
ground-truth `data.frame` (`sequence.i`, `motif.i`, `start`, `width`,
`strand`, `planted`).
