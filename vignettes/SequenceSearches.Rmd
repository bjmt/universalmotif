...
sequences more often than chance?" on the unfiltered 2x2; so the
...
Two scanning-related functions are provided: `scan_sequences()` and `enrich_motifs()`. The latter simply runs `scan_sequences()` twice on a set of target and background sequences. Given a motif of length `n`, `scan_sequences()` considers every possible `n`-length subsequence in a sequence and scores it using the PWM format. If the match surpasses the minimum threshold, it is reported. This is the case regardless of whether one is scanning with a regular motif, or using the higher-order (`multifreq`) motif format (the `multifreq` matrix is converted to a PWM).
...
the same hit set, but skips all of the optional R-side bookkeeping
that `scan_sequences()` does to support its broader API.
...
### Speed comparison vs `motifmatchr` and `yamtk scan` {.unnumbered}
...
the cli tool [`yamtk`](https://github.com/bjmt/yamtk) (by
the same author as the author of `universalmotif`),
...
[`yamtk enr`](https://github.com/bjmt/yamtk). It is built on
...
cleanly onto `enrich_motifs2()`'s surface, a one-line hint is emitted
...
[`yamtk me`](https://github.com/bjmt/yamtk). For each motif width in a
...
which plug directly into `view_motifs()`, `compare_motifs2()`,
