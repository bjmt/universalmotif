...
test_that("Results are accurate", {
  ...
  expect_equal(res$start[1], 3)
  expect_equal(res$motif[1], "motif")
  expect_equal(res$sequence[1], "1")
  expect_equal(res$stop[1], 6)
  expect_equal(res$match[1], "GAAA")
  expect_equal(res$strand[1], "+")
  expect_equal(res$score[1], -0.691, tolerance = 0.001)
  expect_equal(res$`max.score`[1], 7.957, tolerance = 0.001)
  expect_equal(res$`score.pct`[1], 75, tolerance = 0.1)

  expect_equal(res$score[1], res$score[3])
  expect_equal(res$`max.score`[1], res$`max.score`[2])
  expect_equal(res$strand[5], "-")
  expect_equal(res$start[5], 14)
  expect_equal(res$stop[5], 11)
