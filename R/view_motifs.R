...
# TODO: For the Roboto Medium font, the I's get stretched out to x0=0, x1=1,
# which reults in some ugly looking I's...
# At the very least, show an example where the I's are manually shrunk:
#
# library(universalmotif)
# data(fontDFroboto)
# fontdf <- as.data.frame(fontDFroboto)
# tofix <- fontdf$x[fontdf$group == "I"]
# tofix <- tofix * 0.35 + 0.5 - 0.35 / 2
# fontdf$x[fontdf$group == "I"] <- tofix
# view_motifs2(create_motif(alphabet = "AA"), fontDF = fontdf)

# Or scale the letters one at a time in the make_letter function from 0 to 1
# (y, not x).
...
view_motifs <- function(motifs, use.type = "ICM", method = "ALLR",
  ...
  colour.scheme = NULL, fontDF = NULL, min.height = 0.03, x.spacer = 0.04,
...
make_position_polygon_data <- function(vec, fontDF, min.height = 0.03,
  ...
  if (all(vec == 0)) return(
    data.frame(x = NA_real_, y = NA_real_, order = NA_integer_,
      group = NA_character_, pathGroup = NA_character_,
      letter.id = NA_character_)
  ...
  # TODO: The current y.spacer implementation is technically introducing some
  ...
  # TODO: Make it so the y.spacer is implemented between the bottom letter of
